...
# Conv Layer 4
    # Negative dimension size caused by subtracting 3 from 2
        # for 'conv3d_4/Conv3D'(op: 'Conv3D') with
        # input shapes: [?, 6, 6, 2, 256], [3, 3, 3, 256, 256].
...
# Deep Layers
        ...
        # Define the model.
        model = keras.Model(inputs, outputs, name="ALight3D_FINAL")
        return model
    ...
    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(
        "Alight3D_final_CB_CP.h5", save_best_only=True)
    ...
    model.save(os.path.join(model_dir, 'ALight_3D_FINAL.h5'))
